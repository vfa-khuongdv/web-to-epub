...
### Named Rules
**The Two Reserved Colors Rule.** Blue means selection, live work, or a live watch — new chapters found, or a watch toggle that is on; red means failure. Nothing else in the interface may be blue or red, and neither may be used decoratively, for emphasis, or for warning. Every remaining surface is a neutral gray.
...
- **Toggle on:** a watch toggle that is on (the bell in a library row or the story detail) adds no chrome — no fill, no rule. Its glyph fills and turns Instrument Blue Deep, and its label follows; off returns it to the outline glyph and neutral text. The filled glyph is the toggle's whole read.
